...
//     skip:true
//   })
// const dashboardData = getDashboard?.data?.weekly_booking_analytics
...
export default function WeeklyBookingGraph() {
  ...
  // Map the API data to the format expected by BarChart
  const chartData = dashboardData?.map((item: { day: string, booked: number }) => ({
    name: item.day,   // Day of the week
    bookings: item.booked  // Bookings for that day
  })) || []
  ...
  return (
    <Card className="bg-[#1e1829] text-white border-none rounded-xl shadow-lg h-full">
      ...
      <CardContent className="h-[300px] md:h-[350px] lg:h-[400px] xl:h-[350px] pb-0">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
            data={chartData}  // Use the dynamic data here
            margin={{
              top: 10,
              right: 30,
              left: 0,
