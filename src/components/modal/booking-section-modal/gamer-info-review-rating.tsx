...
const GamerInfoReviewRating = ({ open, setIsOpen, bookingDetails }: RatingProps) => {
  const maximumRating = bookingDetails?.rating?.rating 
  ...
  return (
    <div className="p-4">
...

      {
        maximumRating && <div>
          ...
          <div className="flex items-center space-x-3 py-4">
            {[...Array(maximumRating)].map((_, index) => (
      ...
      {bookingDetails?.rating?.review && <div className="mt-8">
        <h2 className="font-bold text-xl">Review</h2>
        <p className="text-sm text-gray-400">{bookingDetails?.rating?.review}</p>
      </div>}
